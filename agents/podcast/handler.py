...
def _ensure_localized_feed_title(title: str, script: str, episode_dir: Path, lang: str) -> None:
    ...
    try:
        ...
        prompt = f"""为中文播客写一个单集标题。不要翻译英文标题，要按中文听众会点开的方式重新命名。

要求：
- 8-18个汉字左右，口语、锋利、有悬念
- 像中文播客标题，不像论文标题、机器翻译标题、公众号标题党
- 可以完全重写，但必须抓住节目最核心的矛盾
- 避免直译结构，比如“为什么你的X会Y你”“X已经做了工作”“模型同意你”
- 优先使用自然中文动词：看错、误判、复查、露馅、失灵、一起错
- 只输出标题，不要解释，不要引号
