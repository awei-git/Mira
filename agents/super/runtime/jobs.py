...
BACKGROUND_JOBS: list[JobSpec] = [
    ...
    JobSpec(
        ...
        command=[],
        trigger="cooldown",
        trigger_name="should_comparative_book_project",
        window_start=10,
        window_end=14,
        state_key_pattern="comparative_book_project_{date}",
        priority=35,
        blocking_group="heavy",
        bg_name_pattern="comparative-book-project",
        launcher="script",
        script_path="../reader/comparative_book_project.py",
        description="30-day comparative book reading project",
    ),
    JobSpec(
        name="skill-study",
    ...
    JobSpec(
        name="health-weekly",
        command=["health-weekly"],
        trigger="time_window",
        trigger_name="_should_health_weekly_report",
        window_start=9,
        window_end=12,
        state_key_pattern="health_weekly_{date}",
        inline=True,
        inline_runner="health-weekly",
...
def build_job_dispatch(
    job: JobSpec, payload, python_executable: str, core_path: str, user_id: str | None = None
) -> tuple[str, list[str]]:
