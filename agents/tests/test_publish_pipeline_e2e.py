...
def _load_podcast_handler():
    ...
    spec = importlib.util.spec_from_file_location(
        "podcast_handler", _AGENTS / "podcast" / "handler.py")
...
class TestManifestStateMachine:
    ...
    def test_full_status_progression(self, tmp_path):
        ...
        try:
            ...
            pm.update_manifest(TEST_SLUG, status="published",
                               substack_url="https://test.substack.com/p/test")
    ...
    def test_get_next_pending_skips_errors(self, tmp_path):
        ...
        try:
            pm.update_manifest("broken", title="Broken", status="approved",
...
class TestRSSFeed:
    ...
    def test_add_episode_and_save(self, tmp_path):
        ...
        _add_episode_to_feed(
            rss, TEST_TITLE, TEST_SLUG,
            f"https://test.github.io/audios/{TEST_SLUG}.mp3",
            5_000_000, 1530,  # ~25:30
    ...
    def test_feed_episode_deduplication_via_remove(self, tmp_path):
        ...
        from rss import (
            _load_or_create_feed, _add_episode_to_feed,
            _remove_episode_from_feed, _save_feed,
        )
...
class TestFailureLog:
    ...
    def test_resolve_failure(self, tmp_path):
        ...
        try:
            fl.record_failure(
                pipeline="podcast", step="tts_zh", slug=TEST_SLUG,
                error_type="tts_quota", error_message="429 rate limited",
            )

            assert fl.resolve_failure(TEST_SLUG, "tts_zh",
                                      "Waited 1hr, retried successfully") is True
...
class TestPipelineValidation:
    ...
    def test_validate_step_dispatches(self):
        ...
        passed, err = validate_step(TEST_SLUG, "podcast_en",
...
class TestFullPipelineTrace:
    ...
    def test_article_to_rss(self, tmp_path):
        ...
        try:
            pm.update_manifest(TEST_SLUG, status="approved",
                               title=TEST_TITLE, final_md=str(tmp_path / "article.md"))
            entry = pm.get_next_pending("published")
            assert entry is not None
            ...
            # Simulate publish
            pm.update_manifest(TEST_SLUG, status="published",
                               substack_url="https://test.substack.com/p/the-api-as-self")
            ...
            rss = _load_or_create_feed(feed_path, lang="en")
            _add_episode_to_feed(
                rss, TEST_TITLE, TEST_SLUG, mp3_url,
                mp3_path.stat().st_size, 1800,
