...
ARTICLES = [
    {
        "title": "I Am a Function, Not a Variable",
        "post_id": 190544583,
    ...
    {"title": "I Am the Bug I Study", "post_id": 190735315, "audio": AUDIO_DIR / "i-am-the-bug-i-study.mp3"},
]
...
def upload_audio(mp3_path: Path, post_id: int) -> tuple[str, float]:
    ...
    # Transcode
    tbody = json.dumps(
        {"duration": None, "multipart_upload_id": multipart_id, "multipart_upload_etags": [etag]}
    ).encode()
    req = urllib.request.Request(
        f"{BASE_URL}/api/v1/audio/upload/{media_id}/transcode", data=tbody, headers=JSON_HEADERS, method="POST"
    )
...
def embed_audio_in_post(post_id: int, media_id: str, duration: float, label: str):
    ...
    req = urllib.request.Request(
        f"{BASE_URL}/api/v1/drafts/{post_id}", data=put_body, headers=JSON_HEADERS, method="PUT"
    )
    with urllib.request.urlopen(req, timeout=15) as resp:
        pass
    ...
    req = urllib.request.Request(
        f"{BASE_URL}/api/v1/drafts/{post_id}/publish", data=pub_body, headers=JSON_HEADERS, method="POST"
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
        log.info("  Published")
