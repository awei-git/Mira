...
def make_sharpen_params(radius: float = 0.8, amount: float = 0.7, threshold: float = 0.5) -> str:
...
def build_wa_style_xmp(base_xmp_path: Path, output_path: Path, ev: float = 0.25) -> bool:
    ...
    xmp = re.sub(r'darktable:history_end="\d+"', f'darktable:history_end="{count}"', xmp)
...
def _replace_module_params(xmp: str, operation: str, new_params: str) -> str:
    ...
    pattern = rf'(darktable:operation="{operation}".*?' rf'darktable:params=")([^"]*?)(")'
    return re.sub(pattern, rf"\g<1>{new_params}\3", xmp, flags=re.DOTALL)


def _append_module(xmp: str, operation: str, modversion: int, params: str, enabled: int = 1) -> str:
