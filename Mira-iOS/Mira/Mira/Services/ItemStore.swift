...
final class ItemStore {
    private(set) var items: [MiraItem] = []
    ...
    var needsAttention: [MiraItem] {
        items.filter(\.needsAttention).sorted { $0.date > $1.date }
    }

    var activeRequests: [MiraItem] {
    ...
    func filtered(type: ItemType? = nil, tag: String? = nil, status: ItemStatus? = nil) -> [MiraItem] {
    ...
    func search(_ query: String) -> [MiraItem] {
    ...
    func item(for id: String) -> MiraItem? {
