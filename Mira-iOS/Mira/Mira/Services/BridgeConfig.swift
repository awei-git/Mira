...
final class BridgeConfig {
    ...
    var profile: MiraProfile?
    var profiles: [MiraProfile] = []

    var isSetup: Bool { bridgeURL != nil }
    var isProfileSelected: Bool { profile != nil }
    var agentName: String { profile?.agentName ?? "Mira" }
    ...
    var heartbeatURL: URL? { bridgeURL?.appending(path: "heartbeat.json") }
    ...
    func selectProfile(_ p: MiraProfile) {
        profile = p
