...
def _seed_bridge_layout(tmp_path):
    ...
    (users / "default" / "items").mkdir(parents=True)
...
def test_read_incoming_is_passive(tmp_path):
    ...
    adapter = NotesBridgeAdapter(bridge_dir=tmp_path, user_id="default")
...
def test_send_outgoing_creates_new_item(tmp_path):
    ...
    items = list((tmp_path / "users" / "default" / "items").glob("outbound-1*"))
...
def test_send_outgoing_appends_to_existing_item(tmp_path):
    ...
    first = BridgeMessage(id="thread-1", user_id="default", source="notes", content="first reply")
...
def test_heartbeat_missing_file_returns_epoch(tmp_path):
    users = tmp_path / "users" / "default" / "items"
