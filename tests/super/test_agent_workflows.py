...
def test_coder_agent():
    ...
    result = _call_agent(
        "coder", "Write a Python function that checks if a string is a palindrome. Just the code, no explanation."
    )
...
def test_socialmedia_agent():
    ...
    result = _call_agent(
        "socialmedia", "Draft a Substack note about learning to code. DO NOT publish, just return the draft text."
    )
...
Reply with ONLY the agent name, nothing else."""
        ...
        assert expected_agent in result, f"Routing '{msg}' → expected '{expected_agent}', got '{result}'"
