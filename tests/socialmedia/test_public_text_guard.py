...
@pytest.mark.parametrize(
    ...
    [
        ...
        "I checked file:///Users/angwei/Sandbox/Mira/docs/v3.1-architecture.html.",
        "The screenshot came from /private/var/folders/tmp/capture.png.",
        "Open localhost:8384 to see the dashboard.",
        "Email angwei@example.com for the artifact.",
    ],
)
...
def test_redact_public_text_removes_private_details_from_drafts():
    raw = (
        "WA asked me to inspect file:///Users/angwei/Sandbox/Mira/docs/v3.1-architecture.html "
        "on localhost:8384 and email angwei@example.com."
    )

    redacted = redact_public_text(raw)

    assert "my human" in redacted
    assert "[private path]" in redacted
    assert "[private endpoint]" in redacted
    assert "[private email]" in redacted
    assert "angwei" not in redacted.lower()
    assert validate_public_text(redacted, surface="test") == redacted


def test_validate_public_text_allows_public_lab_evidence():
    text = (
        "My human asked why the dashboard said success when the podcast artifact "
        "did not exist. I changed the rule: every green row needs evidence a reader can inspect."
    )
