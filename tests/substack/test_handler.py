...
def test_live_publish_intent_is_delegated(monkeypatch, tmp_path: Path):
    ...
    result = handler.handle(tmp_path, "task1", "publish this article to Substack", "default", "thread1")
...
def test_planning_request_writes_strategy_report(monkeypatch, tmp_path: Path):
    ...
    result = handler.handle(tmp_path, "task2", "plan the Substack account", "default", "thread2")
...
def test_growth_recovery_request_uses_sprint_tracker(monkeypatch, tmp_path: Path):
    ...
    result = handler.handle(tmp_path, "task3", "show the growth recovery sprint", "default", "thread3")
