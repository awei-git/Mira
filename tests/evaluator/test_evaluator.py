...
def test_manifest_valid():
    manifest = json.loads((_AGENTS / "evaluator" / "manifest.json").read_text(encoding="utf-8"))
...
def test_all_agents_have_criteria():
    ...
    for name in registry.list_agents():
        if name in exempt:
            continue
        assert name in AGENT_CRITERIA, f"Agent '{name}' has no evaluation criteria in AGENT_CRITERIA"
...
def test_criteria_have_metrics():
    ...
    for name, criteria in AGENT_CRITERIA.items():
        ...
        assert len(criteria["metrics"]) >= 2, f"{name} has only {len(criteria['metrics'])} metrics (need 2+)"
...
def test_no_llm_self_eval_in_scoring():
    ...
    for func_name in score_funcs:
        ...
        assert "claude_think" not in func_body, f"{func_name} should not call claude_think (must be deterministic)"
        assert "claude_act" not in func_body, f"{func_name} should not call claude_act (must be deterministic)"
