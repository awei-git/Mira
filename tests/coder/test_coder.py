...
def _load_coder_handler():
    ...
    handler_path = _AGENTS / "coder" / "handler.py"
...
def test_manifest_valid():
    ...
    manifest_path = _AGENTS / "coder" / "manifest.json"
...
def test_skills_index_valid():
    ...
    skills_dir = _AGENTS / "coder" / "skills"
...
def test_skills_have_trigger_sections():
    ...
    for md in skills_dir.glob("*.md"):
        ...
        has_actionable = (
            "## Decision" in content or "## Method" in content or "## Rules" in content or "## Scan" in content
        )
...
"""
    ...
    lower = result.lower()
    assert "zero" in lower or "empty" in lower or "len" in lower, f"Didn't identify the bug: {result[:300]}"
...
def test_quick_script():
    ...
    result = _run_coder(
        "Write a Python script that reads a CSV file and prints the row count. Just the script, keep it simple."
    )
    assert result, "Coder returned empty"
