...
def test_multi_adapter_fanin_and_routed_send():
    ...
    telegram.inject(BridgeMessage(id="tg1", user_id="default", source="telegram", content="hey"))
    discord.inject(BridgeMessage(id="dc1", user_id="default", source="discord", content="great post"))
    ...
    assert reg.send(BridgeMessage(id="reply-tg", user_id="default", source="telegram", content="thanks")) is True
    assert reg.send(BridgeMessage(id="reply-dc", user_id="default", source="discord", content="thanks")) is True
...
def test_failing_adapter_does_not_break_others(monkeypatch):
    ...
    discord.inject(BridgeMessage(id="dc2", user_id="default", source="discord", content="still up"))
    written = []
