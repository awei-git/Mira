...
更新时间：2026-06-18
状态：canonical，V4.0-aligned

> V4.0 alignment: `docs/north-star.md` defines the canonical goal, `docs/v4-architecture.md` defines the current execution plan, and this file defines durable system boundaries. Any older section that conflicts with the V4.0 L0-L4 stack should be interpreted through the V4.0 stack.
...
核心使命：成为 A2H/A2A trust 领域的独立、受治理的 AI 研究伙伴；在真实运行中存活，从失败中学习，把 agent-human 和 agent-agent 的摩擦转化成实验、工具、公共表达和商业选项。

当前 production scope：

1. L0 Survival：单机常驻运行、worker 隔离、truthful heartbeat、recovery。
2. L1 Memory Compounding：过去经验进入未来行为，有 causal trace。
3. L2 Research-Build：A2H/A2A trust research queue、实验、prototype、GitHub artifact。
4. L3 Public Influence：Substack、X Articles、podcast、GitHub public artifact 的反馈循环。
5. L4 Business Optionality：产品 thesis、customer discovery、合作和收入选项。
...
Mira 应逐步收敛为 9 层。前 7 层来自原 design，V4.0 新增 Survival Spine 和 Public Influence Surface，并明确 Survival 是所有层的前置。
...
3. 改阶段计划或执行顺序：更新 `CURRENT_PLAN.md` 与 `v4-architecture.md`
...
2. L1 memory compounding：过去经验能因果改变未来行为。
3. L2 research-build loop：围绕 A2H/A2A trust 做实验、工具、模型和观点。
4. L3 public influence：Substack、X Articles、podcast、GitHub artifacts 都必须把验证过的洞察带到外部反馈场。
5. L4 business optionality：把经过验证的 insight 转成产品、合作、客户发现或收入路径。
