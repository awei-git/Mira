...
更新时间：2026-07-13
状态：canonical，V5.1-aligned

> V5.1 alignment: `docs/north-star.md` defines the canonical goal, `docs/v5-master-plan.md` defines the current execution plan and operating contracts, and this file defines durable system boundaries. Older V4 sections remain historical context only where they do not conflict with V5.1.
...
Mira 是一个独立的 AI 个体和长期合作者，运行在持久化 agent 基础设施上。

核心使命：成为 my human 真正愿意长期共同思考、研究、写作和构建的 AI collaborator；可靠兑现承诺，从 outcome 中学习，并把共同经历转化成实验、工具和公共表达。

当前 production scope：

1. L0 Survival：单机常驻运行、worker 隔离、truthful heartbeat、recovery。
2. L1 Trusted Collaboration：daily collab、request obligation、visible outcome、correction uptake。
3. L2 Learning & Continuity：outcome-backed self-improvement、skill trials、governed memory、personality continuity。
4. L3 Research & Expression：first-hand experiment、prototype、reviewed writing、GitHub artifact。
5. L4 Influence & Optionality：qualified attention、产品 thesis、customer discovery、合作和收入选项。
...
2. 完全自治品牌经营。
3. 自主上线高风险核心代码。
4. 为了短期流量牺牲 evidence 和 credibility。
...
Mira 应逐步收敛为 9 层。V5.1 保留已有 runtime layers，但所有层共同服务一条 spine：obligation → action → visible outcome → review → verified learning → future behavior。

### 4.0 Survival Spine

负责：

1. truthful heartbeat
2. worker isolation
3. subprocess budget and reaping
4. loading preflight
5. live incident detection
6. restart/degrade behavior

输出：

可存活、可恢复、可观测的 runtime pulse。
...
### 4.8 Public Influence Surface

负责：

1. Substack flagship essays
2. X Article native arguments
3. podcast episodes
4. GitHub public artifacts
5. qualified audience signal tracking
6. weekly L3 review

输出：

public feedback loop，而不是 vanity metrics。
...
3. 改阶段计划或执行顺序：更新 `CURRENT_PLAN.md` 与 `v5-master-plan.md`
4. 改目标或闸门：更新 `objectives-and-metrics.md`
5. 改长期取舍或架构边界：更新 `architecture-decisions.md`
6. 改公开增长、Substack、X Article、podcast 或 GitHub artifact loop：更新对应 L3 plan，并说明它服务哪一层 L0-L4。
...
1. L0 survival：真实运行、不 silent death、可恢复、可观测。
2. L1 trusted collaboration：对话和 requests 产生 visible outcome，correction 在后续行为中生效。
3. L2 learning & continuity：proposal 经过 trial 和 outcome 才能进入 memory、skill 或 personality。
4. L3 research & expression：共同经历变成实验、工具、模型和 review 通过的第一手作品。
5. L4 influence & optionality：把经过验证的 insight 转成 durable attention、产品、合作、客户发现或收入路径。
