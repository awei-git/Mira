...
- 2026-09-25 中午修正：v2 其实共享 v1 会话——从 v1 左侧菜单点 Park Now 进 /v2/parking 是登录态，Active sessions 可见，全流程可走到 Confirm 页。早上"v2 不共享 v1 会话"的结论是错的，错因是直接在地址栏冷启动 /v2/... 网址（冷启动显示未登录并触发 6 位验证码）。路线铁律：永远 v1 → 菜单 → Park Now，绝不冷启动 v2 URL；6 位验证码流程自动化走不通（分离式格子 credential_fill 报 credential_typing_ambiguous 填不进，ticket broken-behavior/otp-code-autofill 已报）。另：平台自带防重复付费——同一车辆有 active session 时 Confirm 页出现红色 Vehicle in use 横幅且 Start parking 按钮灰掉（2026-09-25 实测），但铁律不变：先查 Active sessions 再动手。
